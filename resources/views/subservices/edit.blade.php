                    ...
                    <div class="card-body">
                        ...
                        <form action="/subservices/{{ $subservice->id }}" method="POST">
                                ...
                                <select name="service_id" class="form-control">
                                    @foreach ($services as $service)
                                        <option value="{{ $service->id }}"
                                            @if (old('service_id')) @if (old('service_id') == $service->id)
                                                selected @endif
                                        @else @if ($subservice->service->id == $service->id) selected @endif @endif
                                            >{{ $service->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input name="name" type="text" class="form-control" placeholder="Nama Layanan"
                                    value="{{ old('name') ? old('name') : $subservice->name }}">
                            </div>
                            {{ csrf_field() }}
